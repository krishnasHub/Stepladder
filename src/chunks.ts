 ...
 *   E  entry        X  exit         C  trophy spot (one per level is used)
...
const VERTICAL: ChunkDef[] = [
  ...
  },
  {
    ...
    // Flyers three rows apart: four was only ever tuned for Mochi, and Hugsy's
    // lower double jump could not clear it. The top flyer is off to the side so
    // the last hop isn't squeezed under the exit ledge.
    id: 'v_botladder',
    family: 'v',
    difficulty: 3,
    botGated: true,
...
const DIAGONAL: ChunkDef[] = [
  ...
  {
    // Flyers three rows apart, so every Tuffling can stomp its way across.
    id: 'd_botgate',
    family: 'd',
    difficulty: 3,
    botGated: true,
